...
TEST(TestMediaFormatReader, PositiveMediaTimeDropsNegativeVideoPreroll)
{
  RefPtr dataDemuxer = new MockMediaDataDemuxer();
  RefPtr trackDemuxer =
      new MockMediaTrackDemuxer("video/x-test; width=640; height=360");
  ...
  RefPtr<VideoData> video =
      WaitForResolve(proxy->RequestVideoData(TimeUnit::Zero(), false));
  EXPECT_EQ(video->mTime, TimeUnit::Zero());
